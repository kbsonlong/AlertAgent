...
func RegisterRoutes(r *gin.Engine) {
	...
	apiV1 := r.Group("/api/v1")
	{
		...
		users := authenticated.Group("/users")
		{
			...
			users.DELETE("/batch", middleware.RequireRole("admin"), v1.BatchDeleteUsers)
			users.GET("/check/username", v1.CheckUsernameExists)
			users.GET("/check/email", v1.CheckEmailExists)
			...
			users.POST("/:id/roles", middleware.RequireRole("admin"), v1.AssignRoles)
			users.DELETE("/:id/roles", middleware.RequireRole("admin"), v1.RemoveRoles)
			users.PUT("/:id/password", middleware.RequireRole("admin"), v1.ChangePassword)
			users.POST("/:id/password/reset", middleware.RequireRole("admin"), v1.ResetPassword)
			users.POST("/:id/activate", middleware.RequireRole("admin"), v1.ActivateUser)
			users.POST("/:id/deactivate", middleware.RequireRole("admin"), v1.DeactivateUser)
			users.POST("/:id/lock", middleware.RequireRole("admin"), v1.LockUser)
			users.POST("/:id/unlock", middleware.RequireRole("admin"), v1.UnlockUser)
		}
